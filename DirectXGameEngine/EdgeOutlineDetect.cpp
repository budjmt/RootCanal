...
EdgeOutlineDetect::EdgeOutlineDetect( UINT width, UINT height, ID3D11Device* _device, ID3D11DeviceContext * _deviceContext )
    : PostProcessBase()
{
    device = _device;
    ...
    edgePS->LoadShaderFile( L"EdgeOutlinePS.cso" );
...
void EdgeOutlineDetect::setBlur( bool _blur ) {
    blur = _blur;
}

void EdgeOutlineDetect::setSamples( int _samples ) {
    blurSamples = _samples;
}

void EdgeOutlineDetect::setOutlineWidth( float _width ) {
    outlineWidth = _width;
}

SRV* EdgeOutlineDetect::draw( SRV* ppSRV ) {
